...
.PHONY: help build up down reset rebuild-spark ps logs logs-all health open \
        check check-es check-mongo compile validate-json
...
rebuild-spark: ## Ribuilda e ricrea TUTTE le immagini Spark (job+master+3 worker)
	# I 5 container Spark condividono lo stesso Dockerfile: vanno sempre
	# ribuildati e ricreati insieme, altrimenti i worker restano su un'immagine
	# vecchia (es. senza una nuova dipendenza) e i task falliscono a runtime.
...
check-mongo: ## Conteggio documenti nelle collezioni MongoDB
	@docker exec mongodb mongosh sensor_data --quiet --eval \
		'["raw_readings","processed_readings","node_stats","agg_per_nodo","fire_events"].forEach(c => print("  " + c + ": " + db[c].countDocuments()))' \
		2>/dev/null || echo "  (MongoDB non raggiungibile — stack avviato?)"

# ── Sviluppo ──────────────────────────────────────────────────────────────────

compile: ## Verifica sintassi dei sorgenti Python (Spark job + producer)
	python3 -m py_compile spark_job/spark_stream_job.py csv_producer/csv_producer.py
...
validate-json: ## Valida i dashboard JSON di Grafana
	@for f in grafana/provisioning/dashboards/iot_dashboard.json \
	          grafana/provisioning/dashboards/node_detail_dashboard.json; do \
